...
class CustomLoginsView: CustomBaseView {
    ...
    var index:Int?{
        didSet{
            ...
            index == 1 ? putData() : ()
            mainDropView.isHide(index == 1 ? false : true)
        }
    }
    ...
    lazy var specializationDrop:DropDown = {
        ...
        i.didSelect { (txt, index, _) in
            self.loginViewModel.specificationId = self.specificationIDSArray[index]//index+1
        }
        return i
    }()
    ...
    override func setupViews() {
        ...
        let subStack = getStack(views: phoneNumberTextField,passwordTextField,mainDropView,forgetStack, spacing: 16, distribution: .fillEqually, axis: .vertical)
    ...
    fileprivate func fetchEnglishData(isArabic:Bool) {
        ...
        if isArabic {
            ...
            if let specificationsArray = userDefaults.value(forKey: UserDefaultsConstants.specificationNameARArray) as? [String],let specificationIds = userDefaults.value(forKey: UserDefaultsConstants.specificationIdArray) as? [Int]{
                self.specificationArray = specificationsArray
                self.specificationIDSArray = specificationIds
            }
        }else {
            if let specificationsArray = userDefaults.value(forKey: UserDefaultsConstants.specificationNameArray) as? [String],let specificationIds = userDefaults.value(forKey: UserDefaultsConstants.specificationIdArray) as? [Int] {
                self.specificationArray = specificationsArray
                self.specificationIDSArray = specificationIds
            }
        }
