...
class ClinicDataViewModel {
    ...
    func performUpdating(completion:@escaping (MainDoctorClinicCreateModel?,Error?)->Void)    {
    ...
    func performCancelOrders(completion:@escaping (MainAddFavoriteModel?,Error?)->Void)    {
        guard let clinic_id = clinic_id,let api_token = api_token,let doctor_id=doctor_id else { return  }
        bindableIsResgiter.value = true
        
        DoctorServices.shared.rejectClinicOrderDoctorOfToday(doctor_id: doctor_id, api_token:api_token , clinic_id: clinic_id, completion: completion)
