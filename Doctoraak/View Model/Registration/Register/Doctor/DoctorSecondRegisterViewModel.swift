...
class DoctorSecondRegisterViewModel {
    ...
    func performRegister(completion:@escaping (MainDoctorRegisterModel?, Error?) -> Void)  {
        guard let email = email,let password = password,let name = name,let phone = phone,let img = image,let isInsurance = isInsurance,let cvName=cvName,let cvData=cvFile,let specialization_id=specialization_id,let degree_id=degree_id,let insurance=insurance,let index=index
            else { return  }
        bindableIsResgiter.value = true
        
        RegistrationServices.shared.registerDoctor(index: index, isInsurance: isInsurance, coverImage: img, cvName: cvName, cvFile: cvData, name: name, email: email, phone: phone, password: password, gender: male, specialization_id: specialization_id, degree_id: degree_id, insurance: insurance, completion: completion)
